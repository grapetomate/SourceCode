# -*- coding: utf-8 -*-
"""
Created on Wed Feb 24 17:51:57 2016
...
"""
プログラム説明
・ソフトマージンL1SVMです(L2SVMというのもありますが勉強会ではやってません)
・サポートベクトルおよび正しく分類できていないベクトルX[p]についてはmu[p] != 0を満たします
・ハードマージンSVMからの変更点は63,87,113~132行目の条件分岐のみです
・与えるペナルティξに対応する変数を用意しなくても,双対問題に帰着させれば正しく分類ができます
"""
...
N = 200 #各クラスのデータ数
C = 3 #ペナルティの重さを決めるパラメータ

#カーネル
def kernel(x, y):
    return #np.exp(- 1 * (x-y).T @ (x-y))#x @ y.T
...
def SMO(X, T, theta, mu):
    ...
    #更新するときの条件分岐        
    if T[p] == T[q]:
        if mu[p] + dp > np.min([C, c / T[p]]):
            mup = np.min([C, c / T[p]])
            muq = 0 
        elif mu[p] + dp < np.max([0, c / T[p] - C]):
            mup = np.max([0, c / T[p] - C])
            muq = c / T[q] - mup
        else: #  0 <= mu[p] + dp <= c / T[p]のとき
            mup = mu[p] + dp
            muq = mu[q] - T[p] * T[q] * dp
    else: #T[p] == - T[q]のとき  
        if mu[p] + dp > np.min([C, C + c / T[p]]):
            mup = np.min([C, C + c / T[p]])
            muq = c / T[q] + muq
        elif mu[p] + dp < np.max([0, c / T[p]]):
            mup = np.max([0, c / T[p]])
            muq = c / T[q] + mup
        else: #np.max([0, c / T[p]]) <= mu[p] + dp <= np.min([C, C + c / T[p]]) のとき
            mup = mu[p] + dp
            muq = mu[q] - T[p] * T[q] * dp
    #4
